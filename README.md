...
- [x] Explain benefit(s) using `client-side routing`?
  > Answer: Client side routing is faster than server side routing, it allows the users client to take care of the routing other than the server.
...
- [x] What are `controlled components` in React?
  > Answer: Unlike uncontrolled components, controlled componends controlls the data on the DOM via state.
